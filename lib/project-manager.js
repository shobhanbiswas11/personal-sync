...
class ProjectManager {
  ...
  async init(projectName) {
    ...
    // Create an empty zip file and upload it to S3
    const zipPath = await this.fileManager.createZip([]);
  ...
  async clone(projectName) {
    ...
    const files = fs.readdirSync(this.basePath).filter((file) => {
      // Ignore hidden files and .psync directory
      return !file.startsWith(".") || file === ".psync";
    });

    if (files.length > 0) {
      throw new Error(
        "Cannot clone into non-empty directory. Please use 'pull' command instead."
      );
    }
  ...
  async pull(projectName) {
    ...
    try {
      ...
      console.log(
        chalk.green(`✅ Project "${projectName}" pulled successfully`)
      );
    } catch (error) {
      // Cleanup on failure
      this.fileManager.cleanup(zipPath);
